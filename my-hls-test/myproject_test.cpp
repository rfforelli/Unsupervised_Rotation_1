...
int main(int argc, char **argv)
{
  ...
  if (fin.is_open() && fpr.is_open()) {
    while ( std::getline(fin,iline) && std::getline (fpr,pline) ) {
      ...
      myproject(input_1,layer16_out,size_in1,size_out1);
      ...
      if (e % CHECKPOINT == 0) {
        ...
        nnet::print_result<layer16_t, N_LAYER_16>(layer16_out, std::cout, true);
      }
      e++;

      //hls-fpga-machine-learning insert tb-output
      nnet::print_result<layer16_t, N_LAYER_16>(layer16_out, fout);
